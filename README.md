...
마음돌봄은 '클라우드 없이, 어르신 댁에 놓인 단말 한 대 안에서 모든 AI가 돌아가야 한다'는 제약에서 출발했습니다. 음성 인식부터 언어모델, 비전, 응급 알림까지 한 보드에서 처리해야 했고, 그래서 설계의 모든 선택은 '제한된 자원으로 무엇을 지킬 것인가'라는 질문으로 모였습니다. 첫 관문은 모델이 아니라 플랫폼이었습니다. Jetson Xavier에 운영체제를 올리고 ROS 2와 CUDA 버전을 맞추는 일은 작은 불일치 하나에도 환경 전체가 무너질 만큼 까다로웠고, 본격적인 개발에 앞서 안정적으로 재현되는 빌드 환경을 확보하는 것부터가 하나의 과제였습니다.

가장 큰 난제는 7.8B 규모의 언어모델을 이 보드 위에서 실시간으로 구동하는 일이었습니다. 양자화 수준, GPU에 올리는 레이어 범위, 응답 길이를 번갈아 조정하며 '실사용 가능한 속도'와 '쓸 만한 답변 품질'이 만나는 지점을 찾아야했는데 이는 정해진 답이 있는 문제가 아니라, 측정과 조정을 반복하며 균형점을 직접 잡아 나가는 작업이었습니다. 모델을 그대로 올리면 메모리가 부족했고, 양자화로 용량을 줄이면 이번엔 응답이 느려져 대화의 흐름이 끊겼습니다. 처음엔 7.8B 모델을 4.7GB짜리 `Q4_K_M` GGUF로 올리면 충분히 돌아갈 줄 알았는데, Xavier 위에서는 응답이 늘어지면서 대화 흐름이 자꾸 끊겼습니다. 결국 한 단계 더 가벼운 `Q3_K_M`까지 떨궈 속도를 확보하고, `n_gpu_layers`를 33까지 올려 모델의 모든 레이어를 GPU에 얹는 식으로 VRAM을 끝까지 쥐어짰습니다. 품질을 조금 양보하고서야 비로소 사람의 호흡에 맞는 속도가 나왔습니다. 그런데 속도가 잡히니 이번엔 말투가 문제였습니다. 어르신께 드리는 인사인데 자꾸 챗봇스러운 친절체가 끼어들어, 시스템 프롬프트를 수십 번 갈아엎었습니다. 결국 '1~2문장으로 짧게'라는 한 줄을 페르소나에 못 박고 나서야 비로소 말벗다운 호흡이 잡혔습니다. 

낙상 감지는 정확도와 신뢰성을 동시에 만족시켜야 하는 까다로운 영역이었습니다. 
YOLOv8-pose는 자세 자체를 잘 잡아주었지만, 어르신이 허리를 굽혀 물건을 줍거나 바닥에 앉는 동작까지 낙상으로 판정해버려서 데모만 돌리면 부저가 끊임없이 울었고, 반대로 민감도를 낮추면 실제 낙상을 놓쳤습니다. 

임계값을 올리면 진짜 낙상을 놓치고, 낮추면 헛경보가 멈추지 않았습니다. 그러다 실제 낙상 영상을 모아 돌려보며 한 가지를 알게 됐습니다 — 사람은 넘어지면 보통 그 자리에 몇 초간 가만히 있다는 사실이었습니다. 그 관찰 하나로 구조를 다시 짰습니다. 한 프레임만 보지 않고, 짧은 시간 안에 자세 변화가 모이는지를 본 다음, 뒤이어 정지 상태가 이어지는지 한 번 더 확인하는 2단계 검증으로 바꿨습니다. 모델의 지표를 소수점 둘째 자리 올리려고 애쓰기보다, 시스템이 어떤 흐름으로 판단하는지를 다시 그리는 편이 훨씬 큰 차이를 만든다는 사실을 이때 배웠습니다.

그래서 한 프레임의 자세만으로는 일상 동작과 낙상을 구분할 수 없다고 판단하여, 짧은 시간 구간의 움직임을 모아 보고 쓰러진 뒤의 정지 상태까지 확인하는 2단계 검증으로 구조를 바꿨습니다. 응급 판단에도 같은 원칙을 적용해, 위험 신호가 잡히면 곧바로 경보를 울리는 대신 '괜찮으세요?'라고 먼저 묻는 단계를 두었습니다. 헛경보가 반복되면 보호자가 알림을 신뢰하지 않게 되고, 그 순간 시스템은 존재 이유를 잃기 때문입니다.



마지막까지 발목을 잡은 건 결국 통합이었습니다. 노드 하나하나는 멀쩡히 도는데, 네 개를 Xavier 한 보드에 동시에 띄우면 GPU 메모리가 모자라고 토픽 지연이 누적되면서 TTS가 사용자의 말보다 몇 초씩 늦게 나왔습니다. ROS 2 QoS를 `BEST_EFFORT`로 바꾸고, LLM 노드의 입력 큐를 한 칸으로 잡아 가장 최근 발화만 처리하도록 만들고, 비전 파이프라인을 별도 프로세스로 떼어내는 식으로 하나씩 풀어갔습니다. 

책에서만 보던 '실시간 시스템'이라는 단어가 어떤 무게를 가지는지 처음 손으로 더듬어 알게 된 경험이었습니다. 프로젝트를 마친 지금, 자랑할 만한 결과만 있는 것은 아닙니다. RAG 평가셋이 작아 일반화 성능을 단언하기 어렵고, 낙상 감지도 카메라 각도와 조명에 따라 여전히 흔들립니다. 그래도 분명히 얻은 것이 하나 있습니다. 모델 지표를 0.01 올리는 일보다, 새벽 3시에 어르신이 화장실 가시다 넘어졌을 때 시스템이 보호자에게 정확히 한 번 알릴 수 있느냐가 훨씬 더 중요하다는 사실입니다. 너무 당연해서 문장으로 적으면 싱겁지만, 코드로 직접 부딪혀 깨닫는 과정은 결코 싱겁지 않았습니다.

프로젝트를 마치고 보니, 핵심은 새로운 모델을 만드는 것이 아니라 이미 검증된 기술들을 제한된 하드웨어 안에서 함께 동작하게 만드는 데 있었습니다. 각 기능이 따로 잘 도는 것과, 네 개의 서브시스템이 한 보드 위에서 자원을 나눠 쓰며 끊김 없이 맞물리는 것은 전혀 다른 문제였습니다. 모델의 성능 지표만 들여다보던 관점에서 '이 기술을 실제 환경의 어떤 제약 위에서 동작시킬 것인가'까지 함께 고민하게 된 것이, 이번 프로젝트에서 얻은 가장 큰 수확입니다.
