...
가장 큰 난제는 7.8B 규모의 언어모델을 이 보드 위에서 실시간으로 구동하는 일이었습니다. 양자화 수준, GPU에 올리는 레이어 범위, 응답 길이를 번갈아 조정하며 '실사용 가능한 속도'와 '쓸 만한 답변 품질'이 만나는 지점을 찾아야했는데 이는 정해진 답이 있는 문제가 아니라, 측정과 조정을 반복하며 균형점을 직접 잡아 나가는 작업이었습니다. 모델을 그대로 올리면 메모리가 부족했고, 양자화로 용량을 줄이면 이번엔 응답이 느려져 대화의 흐름이 끊겼습니다. Q4_K_M으로 떨궈도 첫 토큰까지 십수 초가 걸려 대화의 호흡 자체가 무너졌습니다. `n_gpu_layers`를 한 칸씩 올리며 메모리 여유와 응답 속도를 저울질했고, 컨텍스트를 4096에서 2048로 줄이고 나서야 겨우 사람과 비슷한 호흡이 나왔습니다. 그런데 속도가 잡히니 이번엔 말투가 문제였습니다. 어르신께 드리는 인사인데 자꾸 챗봇스러운 친절체가 끼어들어, 시스템 프롬프트를 수십 번 갈아엎었습니다. 결국 '두 문장 이내로 짧게'라는 한 줄을 넣고 나서야 비로소 말벗다운 호흡이 잡혔습니다.


낙상 감지는 정확도와 신뢰성을 동시에 만족시켜야 하는 까다로운 영역이었습니다. 초기 버전은 어르신이 앉거나 허리를 굽히는 동작까지 낙상으로 잡았고, 반대로 민감도를 낮추면 실제 낙상을 놓쳤습니다. 한 프레임의 자세만으로는 일상 동작과 낙상을 구분할 수 없다고 판단해, 짧은 시간 구간의 움직임을 모아 보고 쓰러진 뒤의 정지 상태까지 확인하는 2단계 검증으로 구조를 바꿨습니다. 응급 판단에도 같은 원칙을 적용해, 위험 신호가 잡히면 곧바로 경보를 울리는 대신 '괜찮으세요?'라고 먼저 묻는 단계를 두었습니다. 헛경보가 반복되면 보호자가 알림을 신뢰하지 않게 되고, 그 순간 시스템은 존재 이유를 잃기 때문입니다.
